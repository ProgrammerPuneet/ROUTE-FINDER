  ...
  if(locations.length==0){
    alert("Please Add Locations");
      ...
      for(var x=0;x<locations.length;x++){
          var marker=L.marker([locations[x][1],locations[x][0]],{icon:delivery,draggable:true}).addTo(map);
...
function addItem() {
      ...
      request=new XMLHttpRequest();
      request.open("GET","http://api.positionstack.com/v1/forward?access_key=ce2d4ca9f3432417f0b21845d6c44061&query="+candidate.value);
      request.send();
      request.onload=()=>{
        //console.log(request);
        if(request.status===200){
          ...
          if(r["data"].length==0){
            ...
            var lat=r["data"][0].latitude;
            var long=r["data"][0].longitude;
          ...
          alert("Geocoding Servers are causing this Error");
        }
      }
